...
class WC_Pagarme_API {
	...
	public function process_regular_payment( $order_id ) {
		...
		$refuse_order        = isset( $this->gateway->refused_order ) && 'yes' === $this->gateway->refused_order;

		if ( $is_checkout_pagarme && ! $refuse_order ) {
	...
	public function process_successful_ipn( $posted ) {
		...
		$order_id = absint( $posted['transaction']['metadata']['order_number'] );
		$order    = wc_get_order( $order_id );
	...
	public function process_order_status( $order, $status ) {
		...
		switch ( $status ) {
			...
			case 'refused':
				...
				$this->send_email(
					/* translators: %1$s is replaced with order number */
